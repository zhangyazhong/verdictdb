...
create_sample_stmt :
        T_CREATE T_SAMPLE table_name T_FROM table_name T_SIZE (dec_number | int_number) '%' (T_STORE int_number T_POISSON T_COLUMNS)? (T_STRATIFIED T_BY expr (T_COMMA expr)*)?
        ;

T_STRATIFIED: S T R A T I F I E D;

delete_sample_stmt :
        T_DELETE T_SAMPLE table_name
        ;
...
set_config :
        set_confidence | set_use_samples | set_bootstrap_trials | set_preferred_sample | set_method | set_info | set_sample_type
        ;

set_sample_type:
        T_SET T_SAMPLE T_TYPE (T_UNIFORM|T_STRATIFIED|T_ANY)
        ;

T_ANY: A N Y;
T_UNIFORM: U N I F O R M;

set_info: T_SET T_INFO (T_ON|T_OFF)
        ;

T_INFO: I N F O;

set_method:
        T_SET T_METHOD bootstrap_method
        ;

bootstrap_method:
    T_UDA | T_ON_THE_FLY | T_STORED
    ;

T_METHOD: M E T H O D;
T_UDA: U D A;
T_ON_THE_FLY: O N '-' T H E '-' F L Y;
T_STORED: S T O R E D;

set_confidence:
        T_SET T_CONFIDENCE (dec_number | int_number) '%'?
        ;

T_CONFIDENCE: C O N F I D E N C E;

set_use_samples:
        T_SET T_USE T_SAMPLES (T_ON | T_OFF)
        ;

set_bootstrap_trials:
        T_SET T_BOOTSTRAP? T_TRIALS int_number
        ;

set_preferred_sample:
        T_SET T_PREFERRED? T_SAMPLE  T_SIZE (int_number | dec_number) '%'
        ;

T_PREFERRED: P R E F E R R E D;
T_SAMPLES: S A M P L E S;
T_BOOTSTRAP: B O O T S T R A P;
T_TRIALS: T R I A L S;
...
interval_number :
       T_INTERVAL int_number interval_item
     ;
